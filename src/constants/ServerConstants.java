...
public class ServerConstants {
    ...
    public static final long RESPAWN_INTERVAL = 10 * 1000;                  // 10 seconds, 10,000 milliseconds
    public static final long PURGING_INTERVAL = 5 * 60 * 1000;              // 5 minutes, 300,000 milliseconds
    ...
    public static final boolean USE_SERVER_AUTOASSIGNER = false;            // Custom-builtin autoassigner, uses algorithm based on distributing AP accordingly with required secondary stat on equipments.
    public static final boolean USE_REFRESH_RANK_MOVE = true;
    public static final boolean USE_ENFORCE_ADMIN_ACCOUNT = true;           // Forces accounts having GM characters to be treated as a "GM account" by the client (localhost). Some of the GM account perks is the ability to FLY, but unable to TRADE.
    ...
    public static final boolean USE_ENFORCE_MDOOR_POSITION = true;          // Forces mystic door to be spawned near spawnpoints.
    public static final boolean USE_SPAWN_LOOT_ON_ANIMATION = true;         // Makes loot appear some time after the mob has been killed (following the mob death animation, instead of instantly).
    ...
    public static final boolean USE_JOINT_CASHSHOP_INVENTORY = false;       // Enables usage of a same cash shop inventory for explorers, cygnus and legends. Items from exclusive cash shop inventories won't show up on the shared inventory, though.
    ...
    public static final float PARTY_BONUS_EXP_RATE = 1.0f;                  // Rate for the party exp reward.
    ...
    public static final boolean USE_ANTI_IMMUNITY_CRASH = false;            // Crash skills additionally removes the mob's invincibility buffs. Suggestion thanks to Celestial.
    ...
    static {
        ...
        try {
            ...
            FilePrinter.printStackTrace(ex);
