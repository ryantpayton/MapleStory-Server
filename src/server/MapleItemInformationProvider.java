...
public class MapleItemInformationProvider {
    ...
    // Revamped testYourLuck by David A.
            ...
            if (((nEquip.getUpgradeSlots() > 0 || ItemConstants.isCleanSlate(scrollId))) || assertGM) {
                ...
                if (assertGM || rollSuccessChance(prop)) {
                    ...
                    if (!ItemConstants.isCleanSlate(scrollId)) {
                        // Issue with modifier scrolls taking slots found. Thanks to Masterrulax, justin, and BakaKnyx.
        ...
        if (stats != null) {
            for (Entry<String, Integer> stat : stats.entrySet()) {
                if (stat.getKey().equals("STR")) {
                    ...
                    // Thanks Hyun and Thora for showing an issue with more than only "Untradeable" items being flagged as such here
    ...
    public Collection<Item> canWearEquipment(MapleCharacter chr, Collection<Item> items) {
        ...
        // Removed because players shouldn't even get this, and GMs should just be GM job.
        //try {
        ...
        for (Item item : items) {
            ...
            int reqLevel = getEquipLevelReq(itemId);
            ...
            if (equipStats == null) {
                System.out.println("Could not get stats for equip [" + itemId + "].");
                continue;
            }
            ...
            //    // GMs should just be a GM job, and players cannot change jobs.
            //}
    ...
    public boolean canWearEquipment(MapleCharacter chr, Equip equip, int dst) {
        ...
        if (ItemConstants.isWeddingRing(id) && chr.hasJustMarried()) {
            // Will dc everyone due to doubled couple effect
    ...
    public int getMakerDisassembledFee(Integer itemId) {
        ...
        try {
            ...
            if (rs.next()) {
                // Cost is 13.6363~% of the original value trimmed by 1000
