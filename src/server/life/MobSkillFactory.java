...
public class MobSkillFactory {
    ...
    public static MobSkill getMobSkill(final int skillId, final int level) {
        ...
        try {
            ...
            if (ret == null) {
                MapleData skillRoot = null;
                MapleDataDirectoryEntry root = dataSource.getRoot();

                for (MapleDataDirectoryEntry topDir : root.getSubdirectories()) {
                    String topDirName = topDir.getName();

                    if (topDirName.equals("MobSkill")) {
                        for (MapleDataFileEntry iFile : topDir.getFiles()) {
                            String iFileName = iFile.getName();

                            if (iFileName.equals(skillId + ".img")) {
                                skillRoot = dataSource.getData(topDirName + "/" + iFileName);
                                break;
                            }
                        }
                ...
                if (skillRoot != null) {
                    MapleData skillData = skillRoot.getChildByPath("level/" + level);
