...
public class MaplePacketCreator {
    ...
    private static void addCharacterInfo(final MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        addAreaInfo(mplew, chr); // Assuming it stayed here
    ...
    private static void addCharEntry(final MaplePacketLittleEndianWriter mplew, MapleCharacter chr, boolean viewall) {
        ...
        mplew.write(1);                         // World rank enabled (Next four ints are not sent if disabled) Short??
    ...
    protected static void addItemInfo(final MaplePacketLittleEndianWriter mplew, Item item, boolean zeroPosition) {
        ...
        if (isPet) {
            ...
            mplew.writeInt(pet.getPetFlag()); // Pet flags found by Irenex and Spoon
        ...
        mplew.writeShort(equip.getWatk());              // Weapon Attack
        mplew.writeShort(equip.getMatk());              // Magic Attack
        mplew.writeShort(equip.getWdef());              // Wweapon Defense
        mplew.writeShort(equip.getMdef());              // Magic Defense
    ...
    public static byte[] getAfterLoginError(int reason) {
        ...
        mplew.writeShort(reason); // Using other types than stated above results in a crash
    ...
    public static byte[] sendPolice() {
        ...
        mplew.write(0); // Value doesn't matter
    ...
    public static byte[] getTempBan(long timestampTill, byte reason) {
        ...
        mplew.writeInt(0);
        mplew.write(reason);
        mplew.writeLong(getTime(timestampTill)); // Tempban date is handled as a 64-bit long, number of 100NS intervals since 1/1/1601.
    ...
    public static byte[] getAuthSuccess(MapleClient c) {
        ...
        mplew.writeBool((ServerConstants.USE_ENFORCE_ADMIN_ACCOUNT || canFly) ? c.getGMLevel() > 1 : false);    // Thanks Steve (kaito1410) for pointing out the GM account boolean here
        mplew.write(((ServerConstants.USE_ENFORCE_ADMIN_ACCOUNT || canFly) && c.getGMLevel() > 1) ? 0x80 : 0);  // Admin Byte. 0x80,0x40,0x20..
    ...
    /**
     * Gets a packet detailing a server and it's channels
     *
     * @param serverId
    ...
    public static byte[] getServerList(int serverId, String serverName, int flag, String eventmsg, List<Channel> channelLoad) {
        ...
        mplew.write(100);   // rate modifier
        mplew.write(0);     // event xp * 2.6
        mplew.write(100);   // rate modifier
     ...
     * </p>
     *
     * @param c        The MapleClient to load characters on
     ...
     * @param chr        The character warping <code>to</code>
    ...
    public static byte[] spawnSummon(MapleSummon summon, boolean animated) {
        ...
        mplew.write(summon.getStance());    // bMoveAction and foothold, found thanks to Rien dev team.
        mplew.writeShort(0);
    ...
    // Thanks to Arnah
    public static byte[] removeKite(int objectid, int animationType) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getShowExpGain(int gain, int equip, int party, boolean inChat, boolean white) {
        ...
        mplew.write(0);     // This is always a 0
    ...
    public static byte[] throwGrenade(int cid, Point p, int keyDown, int skillId, int skillLevel) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    // Handle floating point representation through packets
     ...
     * 0E: Crash, length needs to be longer.
     */
    ...
    /**
     * <p>
     * Possible values for <code>animation</code>:<br/>
     * 0: expire<br/>
     ...
     * 2: pickup<br/>
     * 4: explode<br/>
     * cid is ignored for 0 and 1<br /><br />
    ...
    /**
     * <p>
     * Possible values for <code>state</code>:<br/>
     * 0:   del ok state<br/>
     * 12:  invalid bday state<br/>
     * 14:  incorrect pic
     * </p>
     *
     * @param cid
    ...
    public static byte[] charInfo(MapleCharacter chr) {
        ...
        for (int i = 0; i < 3; i++) {
            if (pets[i] != null) {
                ...
                mplew.writeInt(pets[i].getItemId());        // Pet id
    ...
    /**
     * <p>
     * It is important that statups is in the correct order (see declaration order in MapleBuffStat) since this method
    ...
    public static byte[] addQuestTimeLimit(final short quest, final int time) {
        ...
        mplew.writeShort(SendOpcode.UPDATE_QUEST_INFO.getValue());
        mplew.write(6);
        mplew.writeShort(1); // Size but when will there be two at the same time? And it won't even replace the old one.
        mplew.writeShort(quest);
        mplew.writeInt(time);
    ...
    public static byte[] giveDebuff(List<Pair<MapleDisease, Integer>> statups, MobSkill skill) {
        ...
        mplew.writeShort(0);    // Wk charges have 600 here?
     ...
     * 2: Trade cancelled by the other character<br/>
    ...
    public static byte[] showBuffeffect(int cid, int skillid, int skilllv, int effectid, byte direction) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.SHOW_FOREIGN_EFFECT.getValue());
        mplew.writeInt(cid);
    ...
    public static byte[] getWhisper(String sender, int channel, String text) {
        ...
        mplew.writeShort(channel - 1); // Channel?
        mplew.writeMapleAsciiString(text);
    ...
    /**
     * <p>
     * Possible values for <code>i</code>:<br/>
     * 0x0A: Inv full
     ...
     * 1/5/6/11/14/19: Your request for a party didn't work due to an unexpected error.<br/>
     * 12: Quit as leader of the party.<br/>
     * 13: You have yet to join a party.<br/>
     * 16: Already have joined a party.<br/>
     * 17: The party you're trying to join is already in full capacity.<br/>
     * 19: Unable to find the requested character in this channel.<br/>
     * 25: Cannot kick another user in this map.<br/>
     * 28/29: Leadership can only be given to a party member in the vicinity.<br/>
     * 30: Change leadership only on same channel.
     * </p>
     *
     * @param message
     * @return The packet
    ...
    public static byte[] showGuildInfo(MapleCharacter c) {
        ...
        // ...and each of their character ids
    ...
    public static byte[] newGuildMember(MapleGuildCharacter mgc) {
        ...
        mplew.writeInt(mgc.getJobId());
        mplew.writeInt(mgc.getLevel());
        mplew.writeInt(mgc.getGuildRank());     // Should always be 5?
        mplew.writeInt(mgc.isOnline() ? 1 : 0); // Should always be 1?
    ...
    // Updated packet structure found thanks to Rien dev team
    public static byte[] catchMonster(int mobOid, byte success) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
     ...
     * <p>
     * Possible values for <code>status</code>:<br/>
     * 1:       Room already closed<br/>
     * 2:       Can't enter due full cappacity<br/>
     * 3:       Other requests at this minute<br/>
     * 4:       Can't do while dead<br/>
     * 5:       Can't do while middle event<br/>
     * 6:       This character unable to do it<br/>
     * 7/20:    Not allowed to trade anymore<br/>
     * 9:       Can only trade on same map<br/>
     * 10:      May not open store near portal<br/>
     * 11/14:   Can't start game here<br/>
     * 12:      Can't open store at this channel<br/>
     * 13:      Can't estabilish miniroom<br/>
     * 15:      Stores only an the free market<br/>
     * 16:      Lists the rooms at FM?<br/>
     * 17:      You may not enter this store<br/>
     * 18:      Owner undergoing store maintenance<br/>
     * 19:      Unable to enter tournament room<br/>
     * 21:      Not enough mesos to enter<br/>
    ...
    public static byte[] hiredMerchantBox() {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.ENTRUSTED_SHOP_CHECK_RESULT.getValue());
        mplew.write(0x07);
    ...
    // Thanks to Arnah
    public static byte[] getPlayerNPC(MaplePlayerNPC npc) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] sendMTS(List<MTSItemInfo> items, int tab, int type, int page, int pages) {
        ...
        mplew.writeInt(pages * 16);     // Testing, change to 10 if it fails
    ...
    public static byte[] showNotes(ResultSet notes, int count) throws SQLException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            mplew.write(notes.getByte("fame")); // Fame
    ...
    public static byte[] getFindReply(String target, int mapid, int MTSmapCSchannel) {
        ...
        mplew.writeInt(mapid);          // -1 if mts else cs
    ...
    /**
     * Sends a UI utility
     * Possible values for <code>ui</code>:<br/>
     * 0x01: Equipment Inventory<br/>
     * 0x02: Stat Window<br/>
     * 0x03: Skill Window<br/>
     * 0x05: Keyboard Settings<br/>
     * 0x06: Quest window<br/>
     * 0x09: Monsterbook Window<br/>
     * 0x0A: Char Info<br/>
     * 0x0B: Guild BBS<br/>
     * 0x12: Monster Carnival Window<br/>
     * 0x16: Party Search<br/>
     * 0x17: Item Creation Window<br/>
     * 0x1A: My Ranking<br/>
     * 0x1B: Family Window<br/>
     * 0x1C: Family Pedigree<br/>
     * 0x1D: GM Story Board<br/>
     * 0x1E: Envelop saying you got mail from an admin<br/>
     * 0x1F: Medal Window<br/>
     * 0x20: Maple Event?<br/>
     * 0x21: Invalid Pointer<br/>
     * default: Crash
     * </p>
     *
     * @param ui
    ...
    /**
     * <p>
     * Possible values for <code>effect</code>:<br/>
     * 0 = Levelup<br/>
     * 6 = Exp did not drop (Safety Charms)<br/>
     ...
     * 15 = Equipment levelup<br/>
     * 16 = Maker Skill Success<br/>
     * 17 = Buff effect with sound<br/>
     * 19 = Exp card [500, 200, 50]<br/>
     * 21 = Wheel of destiny<br/>
     * 26 = Spirit Stone
     * </p>
     *
     * @param effect
     ...
     * 0x06: Mode 0: You have successfully removed the name from the ranks.<br/>Mode 1: You have entered an invalid character name.<br/>
     * 0x10: GM Hide, mode determines whether or not it is on.<br/>
     * 0x1E: Mode 0: Failed to send warning<br/>Mode 1: Sent warning<br/>
     * 0x13: Mode 0: +mapid<br/>Mode 1: +ch (FF = Unable to find merchant)
     * </p>
     *
     * @param type The type
     * @param mode The mode
    ...
    public static byte[] sendDuey(byte operation, List<DueyPackages> packages) {
        ...
        if (operation == 8) {
            ...
            for (DueyPackages dp : packages) {
                ...
                mplew.writeLong(0); // Contains message
    ...
    public static byte[] getDojoInfo(String info) {
        ...
        mplew.writeShort(SendOpcode.SHOW_STATUS_INFO.getValue());
        mplew.write(10);
        mplew.write(new byte[]{(byte) 0xB7, 4}); // Quest ID
        mplew.writeMapleAsciiString(info);
     ...
     * <p>
     * Possible values for <code>type</code>:<br/>
     * 0: <Family> ? has reached Lv. ?.<br/> - The Reps you have received from ? will be reduced in half.
     * 1: <Family> ? has reached Lv. ?.<br/>
     * 2: <Guild> ? has reached Lv. ?.
     * </p>
     *
     * @param type The type
     ...
     * 1: <Family ? is now married. Please congratulate them.
     * </p>
     *
     * @param type The type
    ...
    public static byte[] marriageMessage(int type, String charname) {
        ...
        mplew.writeMapleAsciiString("> " + charname);   // To fix the packet
     ...
     * B1: Disconnect from CS because of 3 wrong coupon codes
     * B2: Expired Coupon
     * B3: Coupon has been used already
     * B4: Nexon internet cafes?
     * BB: Inv full
     * BC: available to purchase by a use at the premium
     ...
     * C7: Only available to users buying cash item
     ...
     * D6: Fresh people cannot gift items
     ...
     * E7: Sorry for the inconvenience
    ...
    // Type cannot be higher than 4 (Rank D), otherwise it will crash.
     ...
     * Data is still missing, so this is pretty binary at the moment in how it behaves.
     *
